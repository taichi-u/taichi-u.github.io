...
- The public run exposed a timing race in the pre-existing layout check: it could measure a visible DOM node before the stylesheet arrived. An intentionally delayed stylesheet reproduced the exact archive coordinates (726.875px unstyled, 580.6875px styled). The check now awaits the stylesheet and fonts before comparing positions across JavaScript initialization; the styled archive position remains 580.6875px before and after.
